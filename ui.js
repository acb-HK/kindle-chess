...
var CPU_DELAY=2000;  /* pause (ms) before the computer moves, so it feels deliberate */
...
function newGame(){
  ...
  if(G.mode==='cpu' && G.humanSide==='b'){ G.thinking=true; setStatus('Computer thinking…'); setTimeout(computerMove,CPU_DELAY); }
}
...
function doMove(m){
  ...
  if(G.mode==='cpu' && G.turn!==G.humanSide){
    G.thinking=true; setStatus('Computer thinking…'); setTimeout(computerMove,CPU_DELAY);
  } else announceTurn();
}
